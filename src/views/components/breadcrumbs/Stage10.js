...
function Stage1() {
  ...
  const handleChange0 = (e) => {
    ...
    myUser[e.target.name] = e.target.value
    setordr(myUser)
    const count = parseFloat(ordr.weightOut) - (parseFloat(ordr.scapIn) + parseFloat(ordr.finishInReceived) + parseFloat(e.target.value) + parseFloat(ordr.wastage))
  ...
  const handleChange01 = (e) => {
    ...
    newadmin[e.target.name] = e.target.value
    setordr(newadmin)
    const count = parseFloat(ordr.weightOut) - (parseFloat(ordr.finishIn) +  parseFloat(ordr.finishInReceived) +  parseFloat(e.target.value) + parseFloat(ordr.wastage))
    console.log(count.toFixed(3))
    setforms01(count.toFixed(3))
  }

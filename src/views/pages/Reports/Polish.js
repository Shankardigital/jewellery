...
const Ghatkarigar = () => {
    ...
    const profiledet = () => {
        ...
        console.log(token)
        const purity = "18"
        axios.post("http://103.186.185.77:5023/omsanthoshjewellery/employeereport/polishreport/getallpolishempreport", { purity },
            {
            ...
            if (res.status === 200) {
                ...
                setempl(res.data.polishArry)
                setempltot(res.data.polishArrTotal)
            }
    ...
    const navigatedata = (data) => {
        sessionStorage.setItem("polrepid", data.employeeId)
        sessionStorage.setItem("polrepprt", data.percentage)
        navigate("/polish-order-wise")
    }
    ...
    return (
        <Fragment>
            <div
                ...
                <Breadcrumbs title='Polish Karigars' data={[{ title: 'Polish Karigars' }]} />
                <Row>
                                    ...
                                    <Table responsive bordered hover id='empTable'>
                                        <thead>
                                            ...
                                            <tr className='text-center' >
                                                <td colSpan="5" className="text-danger">Polish Karigars</td>
                                            </tr>
                                            <tr className='text-center'>
                                         ...
                                         <Link to="/employee-wise"> <Button size='sm' outline color='warning'>
                                          <i class="fa fa-eye" aria-hidden="true"></i> view
                                            ...
                                            <tr className='text-danger'>
                                                ...
                                                <td >
                                                    {empltot.polishTotalWt === undefined ? (
                                                        ""
                                                    ) : (
                                                        <span>{empltot.polishTotalWt}</span>
                                                    )}
                                                </td>
                                                <td >
                                                </td>
                                                <td>
                                                    {empltot.polishFineWt === undefined ? (
                                                        ""
                                                    ) : (
                                                        <span>{empltot.polishFineWt}</span>
                                                    )}
                                                    {/* {empltot.ghatFineWt} */}
                                                </td>
                                            </tr>

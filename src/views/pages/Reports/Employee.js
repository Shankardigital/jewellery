...
const Employee = () => {
    ...
    const docid = sessionStorage.getItem("gtrepid")
    const purity = sessionStorage.getItem("gtrepprt")
    const profiledet = () => {
        const token = datas
        console.log(token)
        const params = {
        ...
        axios.post("http://103.186.185.77:5023/omsanthoshjewellery/employeereport/ghatReport/empwiseghat", params,
            {
            ...
            if (res.status === 200) {
                ...
                setempl(res.data.finalDataGhatEmp)
                setempl1(res.data.ghatTotals)
            }
    ...
    return (
        <Fragment>
            <div
                ...
                <Breadcrumbs title='Employee Wise' data={[{ title: 'Employee Wise' }]} />
                <Row>
                    <div>

                        <Link to="/ghat-karigar" style={{ float: "right" }} > <Button className='m-1' color='info'> <i class="fa fa-arrow-circle-o-left" aria-hidden="true"></i> Back</Button></Link>
                    </div>
                                    ...
                                    <tbody className='text-center'>
                                        {lists.map((data, key) => (
                                            <tr key={key} >
                                                ...
                                                <td>
                                                    {parseFloat(data.receivedWeight) + parseFloat(data.wastage)}
                                                </td>
                                                <td>
                                                    {data.balance}
                                                </td>
                                        ...
                                        <tr className="text-danger">
                                            ...
                                            <td>{empl1.totalIssuedWeight}</td>
                                            <td>{empl1.totalReceivedWt}</td>
                                            <td>{empl1.totalWastage}</td>
                                            <td>{empl1.completeTotal}</td>

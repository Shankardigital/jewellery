...
const Settings = () => {
    ...
    const docid = sessionStorage.getItem("bandrepid")
    const purity = sessionStorage.getItem("bandrepprt")
    const profiledet = () => {
        const token = datas
        console.log(token)
        const params = {
        ...
        axios.post("http://103.186.185.77:5023/omsanthoshjewellery/employeereport/bandiniReport/empwisebandini", params,
            {
            ...
            if (res.status === 200) {
                ...
                setempl(res.data.finalDatabandiniEmp)
                setempl1(res.data.bandiniTotals)
            }
    ...
    return (
        <Fragment>
            <div
                ...
                <Breadcrumbs title='Bandini' data={[{ title: 'Bandini' }]} />
                <Row>
                    <div>

                        <Link to="/sng-bandini" style={{ float: "right" }} > <Button className='m-1' color='info'> <i class="fa fa-arrow-circle-o-left" aria-hidden="true"></i> Back</Button></Link>
                    </div>

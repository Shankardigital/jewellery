...
const Metal = () => {
    ...
    const profiledet = () => {
        const token = datas
        console.log(token)
        axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/metalinhand/getallbasedonpurity",
            {
            ...
            if (res.status === 200) {
                ...
                setcustomer(res.data.metalInHandobj)
            }
    ...
    const metaldata = (data) => {
        sessionStorage.setItem("metalpurity", data.item)
        navigate("/usemetal")
    }
    ...
    return (
        <Fragment >
            <div data-aos="fade-down"
                ...
                <Breadcrumbs title='Metal in Hand' data={[{ title: 'Metal in Hand' }]} />
                <Row >
                                ...
                                <tbody>
                                    {lists.map((data, key) => (
                                        <tr key={key} >
                                            <td>{key + 1}</td>
                                            <td>
                                                <a className='text-primary' onClick={() => { metaldata(data) }}>{data.item}</a>
                                            </td>
                                            <td>{data.total}</td>

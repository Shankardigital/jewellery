...
const Usemetal = () => {
    ...
    const datapur = sessionStorage.getItem("metalpurity")
    const profiledet = () => {
        const token = datas
        const params = {
            percent: datapur
        ...
        console.log(token)
        axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/metalinhand/getempbypurity", params,
            {
            ...
            if (res.status === 200) {
                ...
                setcustomer1(res.data.totalReceivedGold)
                setcustomer2(res.data.totalIssuedGold)
    ...
    return (
        <Fragment>
            <div
                ...
                <Breadcrumbs title='Metal uses' data={[{ title: 'Metal uses' }]} />
                <Row>
                    <div>

                        <Link to="/metal" style={{ float: "right" }} > <Button className='m-1' color='info'> <i class="fa fa-arrow-circle-o-left" aria-hidden="true"></i> Back</Button></Link>
                    </div>

...
const Casting = () => {
  ...
  const actiordrs = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/getallreceivedpolish2ndstage",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/searchreceivedpolish2nd?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setordr(res.data.Polish2ndData)
      }
    }).catch(function (error) {
      if (error.response) {
        console.log(error.response.data.message)
  ...
  const Addcasting = () => {
    ...
    console.log(token)
    axios.put(`http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/editreceivedpolish2nd/${docsid}`, params,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  return (
    <Fragment >
      <div
        ...
        <ExtensionsHeader title="Polish 2nd Stage Received List" />
        ...
        <Row>
          <Card>
            <div >
              <Link to="/polish2-details"> <Button className="m-1" color="primary">

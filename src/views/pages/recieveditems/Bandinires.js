...
//   }
  ...
  const activeitems = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/bandini/getallreceivedbandini",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/bandini/searchbandinireceived?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setactitm(res.data.BandiniData)
      }
        }).catch(function (error) {
        if (error.response) {
            console.log(error.response.data.message)
...
{show ? (
      ...
      <Card>
          <div >
          <Link to="/bandhini-details"> <Button className="m-1" color="primary">

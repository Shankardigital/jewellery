...
//     const params = face
//     console.log(token)
//     axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/setting/filterWithDates", params,
//       {
...
//   }
  ...
  const activeitems = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/setting/getallreceivedsetting",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setactitm(res.data.settingResult)
      }
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/setting/searchreceivedsetting?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setactitm(res.data.SettingData)
      }
        }).catch(function (error) {
        if (error.response) {
            console.log(error.response.data.message)
...
//     sessionStorage.setItem("setid", data._id)
//     sessionStorage.setItem("setitem", data.itemName)
//     sessionStorage.setItem("ordobid5", data.orderId)
//     navigate("/stage4")
//   }
  const getonemmber123 = (data) => {
    sessionStorage.setItem("setobjid", data._id)
    // sessionStorage.setItem("setitem", data.itemName)
    // sessionStorage.setItem("ordobid5", data.orderId)
    navigate("/editsettingres")
  }
  ...
  return (
    <Fragment >
      <div
        ...
        <ExtensionsHeader title="Setting Department" />
        ...
        <Row>
          <Card>
            <div >
            <Link to="/setting-details"> <Button className="m-1" color="primary">

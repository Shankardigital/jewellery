...
//   const ordid = sessionStorage.getItem("ordobid5")
  const settid = sessionStorage.getItem("malaobjid")
...
//   console.log(balanace)
  ...
  const actiordrs = () => {
    ...
    //   itemName: settitem
    // }
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/mala/getmalabyobjid/${docsetid}`,
      {
      ...
      if (res.status === 200) {
        ...
        setordr(res.data.MalaData)
        setforms02(res.data.MalaData.wastage)
        setforms01(res.data.MalaData.balance)
        setnetwet(res.data.MalaData.netWeight)
        // setordrtot(res.data.outWeight)
        // console.log(res.data.outWeight)
        setordr01(res.data.MalaData.updatedOtherDetails)
      }
...
//   }
  ...
  const updateset = () => {
    ...
    console.log(token)
    axios.put(`http://103.186.185.77:5023/omsanthoshjewellery/admin/mala/editreceivedmala/${docsetid}`, params,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  return (
    <Fragment>
      <div
        ...
        <BreadCrumbsPage data={[{ title: "Mala Received List" }]} />

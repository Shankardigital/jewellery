...
const Casting = () => {
  ...
  const face = { submittedDate: datess }

  const cadfilter = () => {
    const token = datas
    const params = face
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/finishing/filterWithDates", params,
      {
  ...
  const actiordrs = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/finishing/getallfinishedorders",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/finishing/getfinishedordersbysearch?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const getonemmber = (data) => {
    sessionStorage.setItem("finordid", data.orderId)
    sessionStorage.setItem("finorditem", data.itemId)
    navigate("/stage1")
  }
  const getonemmber1 = (data) => {
    sessionStorage.setItem("finingdid", data._id)
    sessionStorage.setItem("finorditem", data.itemName)
    navigate("/editfinish")
  }
  ...
  return (
    <Fragment >
      <div
        ...
        <ExtensionsHeader title="Finishing" />
          ...
          <Card>
            <div >
              {access.orderAdd === true || adrole === "admin" ? (
                <Link to="/stage8"> <Button className="m-1" color="primary">
              ...
              <tbody className='text-center'>
                {lists.map((data, key) => (
                  <tr key={key}>
                    ...
                    <td>
                      <img width="80px" src={`http://103.186.185.77:5023/${data.finishingImg}`} />
                    </td>
                    {/* <td>
                      {data.status}
                    </td> */}

...
const Maladetails = () => {
  ...
  const activeitems = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/mala/getallpendingmala",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/mala/getmalabysearch?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
          ...
          <Card>
            <div >
              {access.orderAdd === true || adrole === "admin" ? (
                <Link to="/addmala"> <Button className="m-1" color="primary">

...
const Settingdetails = () => {
  ...
  const activeitems = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/setting/getallpendingsetting",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/setting/getsettingbysearch?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setactitm(res.data.settingFound)
      }
        }).catch(function (error) {
        if (error.response) {
            console.log(error.response.data.message)
          ...
          <Card>
            <div >
              ...
              {access.orderAdd === true || adrole === "admin" ? (
                <Link to="/Addsettings"> <Button className="m-1" color="primary">
              ...
              {access.orderEdit === true || adrole === "admin" ? (
                <Link to="/settingres"> <Button className="m-1" color="warning">

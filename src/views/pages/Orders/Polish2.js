...
const Casting = () => {
  ...
  const cadfilter = () => {
    ...
    const params = face
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/filterWithDates", params,
      {
  ...
  const actiordrs = () => {
    const token = datas
    console.log(token)
    axios.post("http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/getallpendingpolish2ndstage",
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
      if (res.status === 200) {
        console.log(res.data)
        setordr(res.data.polish2ndStageResult)
      }
        }).catch(function (error) {
        if (error.response) {
            console.log(error.response.data.message)
  ...
  const custsearch = (e) => {
    ...
    const token = datas
    console.log(token)
    axios.post(`http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/getpolish2ndstagebysearch?searchQuery=${e.target.value}`,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const Addcasting = () => {
    ...
    console.log(token)
    axios.put(`http://103.186.185.77:5023/omsanthoshjewellery/admin/polish2ndstage/editpolish2ndissued/${docsid}`, params,
      {
        headers: { Authorization: `Bearer ${token}` }
      }, {}
    ).then((res) => {
  ...
  const getonemmber = (data) => {
    sessionStorage.setItem("pols2id", data._id)
    sessionStorage.setItem("pols2item", data.itemName)
    sessionStorage.setItem("ordobid4", data.orderId)
    navigate("/stage11")
  }
  ...
  return (
    <Fragment >
      <div
        ...
        <ExtensionsHeader title="Polish 2nd Stage Details" />
          ...
          <Card>
            <div >
              {access.orderAdd === true || adrole === "admin" ? (
                <Link to="/Addpolish2"> <Button className="m-1" color="primary">
              ...
              {access.orderEdit === true || adrole === "admin" ? (
                <Link to="/polish2ndres"> <Button className="m-1" color="warning">
        ...
        <Modal isOpen={centeredModal} toggle={() => setCenteredModal(!centeredModal)} className='modal-dialog-centered'>
          <ModalHeader toggle={() => setCenteredModal(!centeredModal)}>Edit polish 2nd stage</ModalHeader>
          <ModalBody>
